...
function shouldLog(tag: string): boolean {
  ...
  const filter = localStorage.getItem(LS_DEBUG) ?? "";
  if (!filter) {
    // Check URL param
...
export function getDebugFilter(): string {
  ...
  return localStorage.getItem(LS_DEBUG) ?? "";
}

/** Toggle debug mode on/off, or set a tag filter */
export function setDebugMode(on: boolean | string): void {
  if (typeof on === "string") {
    localStorage.setItem(LS_DEBUG, on);
  } else if (on) {
    localStorage.setItem(LS_DEBUG, "1");
  } else {
    localStorage.removeItem(LS_DEBUG);
  }
  refreshDebugState();
}
