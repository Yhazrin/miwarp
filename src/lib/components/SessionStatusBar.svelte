...
<!--
  ...
  The outer shell deliberately does NOT use data-tauri-drag-region / CSS app-region drag:
  WKWebView on macOS frequently dropped real clicks onto custom controls underneath.
  Use WindowDragArea spacers plus global TopWindowDrag (JS capture) instead.

  Inside the island: Tier 1 = primary controls row; Tier 2 = expandable detail row.
-->
<div
  class="session-island-shell relative mt-3 w-full min-w-0 border border-border/55 dark:border-white/[0.13] bg-background/[0.42] dark:bg-background/[0.34] font-mono text-xs text-foreground/70 backdrop-blur-2xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.38)] dark:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08)] transition-[border-radius,background-color,border-color,box-shadow] duration-[520ms] ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:duration-150 motion-reduce:ease-linear {fuseToolRailCapsule
    ? 'max-w-none'
