...
<div
  class="grid grid-cols-1 min-[520px]:grid-cols-[minmax(0,1fr)_auto] gap-2 min-[520px]:gap-x-5 min-[520px]:items-center py-2.5 group"
>
  <!-- Label -->
  <span class="text-sm text-foreground min-w-0 leading-snug" title={binding.label}
    >{binding.label}</span
  >

  <!-- Key + actions (right-aligned on wide screens) -->
  <div class="flex flex-wrap items-center gap-x-2 gap-y-1.5 min-[520px]:justify-end">
    <!-- Key display / recording -->
    {#if recording}
      <button
        bind:this={buttonRef}
        class="flex items-center gap-1.5 rounded-md border-2 border-blue-500 bg-miwarp-status-info/10 px-3 py-1 text-sm font-mono min-w-[7.5rem] animate-pulse focus:outline-none shrink-0"
    ...
    {:else}
      <span
        class="inline-flex items-center rounded-md border bg-muted/50 px-2.5 py-1 text-xs font-mono text-foreground min-w-[3.25rem] justify-center shrink-0 {isOverridden
