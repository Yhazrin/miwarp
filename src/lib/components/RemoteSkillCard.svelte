  ...
  function badgeClass(status: string) {
    switch (status) {
      ...
      case "update_available":
        return "bg-[hsl(var(--miwarp-status-info)/0.15)] text-miwarp-status-info";
      case "conflict":
        return "bg-[hsl(var(--miwarp-status-warning)/0.15)] text-miwarp-status-warning";
      default:
  ...
  <div class="flex items-start justify-between gap-3">
    <div class="min-w-0">
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <span
          class="text-[10px] font-medium rounded-full px-1.5 py-0.5 bg-[hsl(var(--miwarp-accent-violet)/0.1)] text-miwarp-accent-violet"
      ...
      {#if candidate.skipped && candidate.skipReason}
        <p class="text-[11px] text-miwarp-status-warning mt-1">
