<script lang="ts">
  ...
  import { updateCodexCli, updateMimoCli, type UpdateCliResult } from "$lib/api";
  ...
  async function handleOneClick(tool: CliToolEntry) {
    // UpdateCenter.svelte already calls the unified dispatcher — here we wire
    // the per-tool APIs so the Settings tab works without a parent dialog.
    let result: UpdateCliResult;
    try {
      switch (tool.id) {
        case "claude-code":
          result = await cliUpdateRegistry.installOrUpdate(tool.id);
          break;
        case "codex":
          result = await updateCodexCli();
          break;
        case "mimo":
          result = await updateMimoCli();
          break;
        case "ccswitch":
          result = await cliUpdateRegistry.installOrUpdate(tool.id);
          break;
      ...
      showToast(
        result.success ? t("updateCenter_installDone") : t("updateCenter_installFailed"),
        result.success ? "success" : "error",
      );
      void cliUpdateRegistry.checkTool(tool.id);
    } catch (e) {
        ...
        <div class="rounded-lg border border-border/50 bg-muted/20 px-4 py-2.5 space-y-1">
          ...
          <div class="flex items-center gap-3 pt-1">
            {#if cliUpdateRegistry.canAutoUpdate(tool.id)}
              ...
              >
                {tool.status === "installing" ? t("updateCenter_installing") : actionLabelFor(tool)}
              </button>
            {/if}
            <button
              type="button"
