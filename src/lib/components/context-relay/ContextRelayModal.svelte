...
{#if contextRelayStore.open}
  ...
  >
    <!-- Modal -->
    <div
      class="elevation-3 relative z-50 w-full max-w-xl max-h-[85vh] overflow-hidden flex flex-col rounded-xl border border-[hsl(var(--miwarp-glass-border)/0.25)] animate-modal-in"
    ...
    >
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b border-border/30">
        <h2 class="text-base font-semibold text-foreground">
          {t("contextRelay_title") || "Send to Session"}
        </h2>
        <button
          class="flex h-7 w-7 items-center justify-center rounded-md text-miwarp-text-tertiary transition-colors hover:bg-miwarp-bg-hover hover:text-miwarp-text-primary"
          onclick={handleClose}
