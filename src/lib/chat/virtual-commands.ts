/**
 * Slash-style virtual command handling (copy-last, help, doctor, diff, tasks, ralph, etc.).
 * Kept in a module to shrink chat +page; behavior matches the former inline implementation.
 */
import { goto } from "$app/navigation";
import * as api from "$lib/api";
import type { SessionStore } from "$lib/stores";
import type { Attachment, CliCommand, TimelineEntry } from "$lib/types";
...
export interface VirtualCommandDeps {
  store: SessionStore;
  t: VirtualCommandTranslate;
  appendCommandOutput: (text: string) => void;
  handleRename: (name: string) => Promise<void>;
  sendMessage: (text: string, attachments: Attachment[]) => Promise<void>;
  handlePermissionModeChange: (newMode: string, opts?: { toast?: boolean }) => Promise<boolean>;
  ...
  openPreviewInSidebar: (args: string) => void;
  showSessionInfoSidebar: () => void;
}

export function createHandleVirtualCommand(deps: VirtualCommandDeps) {
  const {
    store,
    t,
    appendCommandOutput,
  ...
  return async function handleVirtualCommand(action: string, args: string) {
