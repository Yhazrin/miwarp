use super::persist::{extract_run_usage, file_mtime_and_size, UsageCacheFile, USAGE_CACHE_VERSION};
...
fn cache_invalidated_when_events_mtime_advances() {
    ...
    let (mtime_at_cache_write, size_at_cache_write) = file_mtime_and_size(&events_path).unwrap();
